...
public class GameDirector : MonoBehaviour
{
    ...
    //    countText.text = ($"JUMP:{3 - _count}");

    //    float length = flag.transform.position.x - car.transform.position.x; //※旗と車の位置を引いて、距離を求めて入れる
    //    if ((length < 2 && !ToriSpriteController.fly) || (_count == 3 && !ToriSpriteController.fly) || dead)//※if () の内容が当てはまらない時に、{ }の内容が実行される（それ以外の時ということ）
    //    {
    ...
    //    }
    //    else if (length >= 0 && !dead) //※車が旗を越していないか
    //    {
